    ...
    if getattr(Timer, "timer_started"):
        ...
        if value >= 30:
            ...
            for i in pause_menu.buttons:
                ...
                setattr(Timer, "timer_started", False)
...
app = Ursina(title="Aim Training Game", borderless=False)
...
menu_parent = Entity(parent=camera.ui, y=0.15)
...
state_handler = Animator(
    {"main_menu": main_menu, "settings_menu": settings_menu, "pause_menu": pause_menu}
)

main_menu.buttons = [
    MenuButton("Play", on_click=start),
    MenuButton(
        "Settings", on_click=Func(setattr, state_handler, "state", "settings_menu")
    ),
    MenuButton("Quit", on_click=Sequence(Wait(0.01), Func(quit))),
]
...
size_slider = Slider(
    text="Circle Spawn Size",
    min=0.1,
    max=2,
    dynamic=True,
    value=float(size_value),
    on_value_changed=set_size,
    parent=settings_menu,
    x=-0.22,
    y=-0.1,
)
...
game_music = Audio(sound_file_name="assets/season3.mp3", loop=True, autoplay=False)
